...
KIRIKUMO_DEMO=1 KIRIKUMO_DEMO_COLUMNS=1 cargo run   # ...opened on the table's column controls
...
7. **Long lists are virtualized from the first commit.** The table is one `uniform_list`, the variable-height wrapped log is one `gpui::list`, and the YAML tab is the toolkit's editor, which virtualizes for itself; a namespace with four thousand pods must not cost four thousand elements.
