...
**The table is live.** The list on screen — and only that one — is followed: a thread reads the watch, bookmarks keep the resume point moving, a `410 Gone` re-lists, a dropped connection backs off, and only the rows whose objects actually moved are formatted again. Its `kubectl get` columns can be resized, reordered or hidden per resource; only those preferences are persisted, never row data. `KIRIKUMO_DEMO=1` has a scripted watch, so the whole path can be exercised without a cluster.
