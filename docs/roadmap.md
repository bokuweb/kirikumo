...
`gpui-component` over the `gpui` rev it owns, at exactly Ginka's and e1's lock (K4), with its `tree-sitter-yaml` feature on. Used from it: `Root`, `Icon`, `Input`, `Tooltip`, `Editor` for the YAML tab, `gpui::uniform_list` for every table, and `gpui::list` for measured log rows. Built here: the header strips, the health marks, the table's own header and cells, the log view, the palette, the pickers.
...
- **The table is live and configurable.** The list on screen — and only that one — is watched: a thread reads the stream, bookmarks keep the resume point moving while nothing happens, a `410 Gone` re-lists and starts again, and a dropped connection backs off from a second to thirty. A normal five-minute timeout reconnects immediately; repeated short connections retain the backoff, and the transient 403 observed while a restarted apiserver restores authorization is retried only during an established watch's recovery. A change reaches the row without anybody pressing refresh, and only the rows whose objects moved are formatted again. Its columns can be resized, reordered and hidden per resource without persisting any row data.
- **A log can be followed and wrapped.** The Logs tab tails a container and keeps reading, with a container picker, a *Wrap* toggle, a *Previous* toggle for the instance before this one, and a literal find that filters and counts. Measured variable-height virtual rows keep wrapping compatible with the 50,000-line bound and tail-following. A workload with a pod template and selector gets the same tab over its matching Pods, newest first, with an explicit Pod picker. A Node gets it over the Pods scheduled there, across namespaces; this is the honest meaning of “Node logs” because Kubernetes has no Node logs subresource. Run and Shell stay Pod-only. The reading is a thread, like a watch's.
...
| --- | --- | --- | --- |
| **M0** | The window | Workspace, tokens, locales, the three-column shell with no title bar, kubeconfig parsing, context list, `/version` handshake, `Scripted` and `KIRIKUMO_DEMO=1` | **Landed and visually signed off.** The default three-column log surface was checked in dark and light appearances against `docs/ui.md`; the check caught and removed the log's light-theme contrast mismatch. |
...
| **M2** | Live | Watches wired to the store with bookmarks and re-list, rows updated in place rather than rebuilt, `⌘F`/`⌘L`/`⌘K` | **Landed and tuned against a restarted `kind` apiserver.** Transport failures stay on the pump, short drops retain exponential backoff, normal timeout ends reconnect immediately, a bounded recovery handles the apiserver's transient startup 403, and a following `410` re-lists before returning to *watching*. |
| **M3** | Pods in depth | Logs following, with find and the previous instance; `metrics.k8s.io` for nodes and pods; owner/child navigation | **Landed.** Long lines wrap by default in a measured variable-height virtual list and can be returned to one line with *Wrap*. Metrics also appear in Pod and Node tables after the optional API first answers, and refresh on their own clock. |
...
| 2026-09-14 | Logs wrap in `gpui::list` measured rows, with *Wrap* as an explicit escape hatch | `gpui::list` now supplies variable-height virtualization, so the old choice between wrapping and the 50,000-row performance bound no longer exists. A pure reconciliation rule preserves measurements and the scroll anchor on append, and remeasures only when filtering, capped-buffer replacement or the toggle changes row heights. Logs and collected command output use `code.bg`; `bg.terminal` remains reserved for the terminal emulator, which keeps light-theme foreground contrast coherent without adding a Kirikumo-only colour. |
| 2026-09-14 | A watch's reconnect policy belongs in `kirikumo-kube::watch`, and a transient 403 is retryable only while recovering an established connection | The old pump reset its backoff as soon as a socket opened and emitted retryable failures to a store that correctly treated failures as terminal, so it did not actually retry and a rapidly dropped socket could spin. A real restart of the `kind` apiserver then exposed a second edge: authorization briefly returned 403 before a resumed watch returned 410. The domain pump now keeps transport failures private, retains 1–30 second backoff across short connections, resets after ten stable seconds, checks stop every 100 ms, permits at most three recovery-only 403 retries, and still stops immediately on an initial 403. |
