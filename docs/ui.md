...
- **Events** — the object's own events, newest first: type as a mark, reason in mono, the message, the count and the age. *No recent events* is one muted line, and is normal: logs are container output, while Events exist only when a Kubernetes component emits one and are retained for a limited, cluster-configured time. An Event resource is already a record about another object, so its detail omits this tab rather than offering the usually empty and recursive question “events about this Event”. Refresh always asks the apiserver again, including after an earlier empty result. The scripted cluster includes both Pod warnings and one NodeReady event so both paths can be inspected without a cluster.
...
- **Logs** — on a Pod, the container picker as chips when there is more than one. On a Deployment, StatefulSet, DaemonSet, ReplicaSet, ReplicationController or Job with a pod template and selector, a first row picks from the matching Pods, newest first, and the next row picks that Pod's container. A Node gets the same explicit picker over Pods whose `spec.nodeName` names it, across namespaces, with each chip written as `namespace/name`. The same toolbar follows: *Follow* (on by default), *Wrap* (on by default), *Previous* (the instance before this one, which is the only place a crash loop's reason survives), and a find box — and under it the lines in a measured, variable-height virtualized list. Wrapping can be disabled to keep every entry on one line; either mode preserves tail-following and the 50,000-line bound. Picker and controls are fixed-height strips above the clipped log viewport, never part of the log's paint area; when the panel is narrow, each strip scrolls horizontally. The toolbar and log use `code.bg`, so their foreground tokens retain contrast in both appearances. Run and Shell remain Pod-only: a workload or Node owns several replaceable Pods, so there is no honest implicit target for an interactive session.
  ...
  - **Wrap** is on by default and can be toggled in the fixed toolbar. Wrapped lines are measured as variable-height virtual rows, so a fifty-thousand-line scrollback still creates elements only for the visible range. Turning wrapping off returns each entry to one line; changing the mode remeasures in place and preserves tail-following.
...
- **Footer — the actions strip.** The writes the object offers (roadmap M4): *Sync* for an idle Argo CD Application; *Scale* and *Restart* for a controller; *Trigger* and *Suspend* or *Resume* for a CronJob; *Cordon* or *Uncordon* and *Drain* for a node; and *Delete* for anything the apiserver lets anyone delete — last, and in the error colour, where a hand does not fall on it. Sync means the whole Application at its configured revision with prune off; it is absent while an operation is active. CronJob suspend/resume changes only `spec.suspend`. Trigger creates one Job from `spec.jobTemplate`, marks it as a manual instantiation, names the CronJob as its controller owner and reports the created Job's name; its permission is `create` on Jobs, not a write on CronJobs. *Drain* is in the error colour too, because it evicts every pod on the node; when it lands, the strip says what became of them (`4 evicted · 1 skipped`), because a drain that quietly left things behind is worse than one that says it did. *Apply* is not here; it lives beside the text it applies.
...
| Logs | `gpui::list` of measured, variable-height mono lines |
