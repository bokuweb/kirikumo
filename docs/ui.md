...
- **Footer — the actions strip.** The writes the object offers (roadmap M4): *Sync* for an idle Argo CD Application; *Scale* and *Restart* for a controller; *Suspend* or *Resume* for a CronJob according to `spec.suspend`; *Cordon* or *Uncordon* and *Drain* for a node; and *Delete* for anything the apiserver lets anyone delete — last, and in the error colour, where a hand does not fall on it. Sync means the whole Application at its configured revision with prune off; it is absent while an operation is active. CronJob suspend/resume changes that one flag and does not create a Job. *Drain* is in the error colour too, because it evicts every pod on the node; when it lands, the strip says what became of them (`4 evicted · 1 skipped`), because a drain that quietly left things behind is worse than one that says it did. *Apply* is not here; it lives beside the text it applies.
